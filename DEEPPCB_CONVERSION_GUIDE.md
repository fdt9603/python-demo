...
# 或者如果下载的是zip文件，解压后会得到 DeepPCB-master 目录
# 数据集在 PCBData/ 目录中
cd DeepPCB  # 或 cd DeepPCB-master
ls PCBData/
```
...
**注意**: DeepPCB数据集使用分组目录结构，转换脚本会自动递归查找所有 `*_test.jpg` 文件，无需手动整理。
...
- **图像文件**: 
  - `xxx_test.jpg` - 带缺陷的测试图像（640x640像素）
  - `xxx_temp.jpg` - 无缺陷的模板图像（640x640像素）
...
脚本会自动查找 `PCBData` 目录，你可以直接指定 `DeepPCB-master` 根目录或 `PCBData` 目录。
...
# 方式2: 直接指定PCBData目录
python convert_deeppcb_dataset.py --deeppcb_dir /path/to/DeepPCB-master/PCBData

# 指定输出目录
python convert_deeppcb_dataset.py \
    --deeppcb_dir /path/to/DeepPCB-master \
    --output_dir ./data/pcb_defects

# 自定义训练集比例（默认80%训练，20%测试）
python convert_deeppcb_dataset.py \
    --deeppcb_dir /path/to/DeepPCB-master \
    --output_dir ./data/pcb_defects \
    --split_ratio 0.9
...
python convert_deeppcb_dataset.py --deeppcb_dir "D:\datasets\DeepPCB-master"

# 或者直接指定PCBData目录
python convert_deeppcb_dataset.py --deeppcb_dir "D:\datasets\DeepPCB-master\PCBData"
```

**Linux/Mac示例**:
```bash
# 如果数据集在 ~/datasets/DeepPCB-master
python convert_deeppcb_dataset.py --deeppcb_dir ~/datasets/DeepPCB-master

# 或者直接指定PCBData目录
python convert_deeppcb_dataset.py --deeppcb_dir ~/datasets/DeepPCB-master/PCBData
```
...
# 或者手动检查
python -c "from data_loader import load_pcb_dataset; d=load_pcb_dataset('data/pcb_defects'); print(f'数据集大小: {len(d)}')"
```
...
with open('data/pcb_defects/labels.json', 'r', encoding='utf-8') as f:
    ...
    print(f"第一个样本: {json.dumps(data[0], ensure_ascii=False, indent=2)}")
```
...
如果需要保留所有6种类型，可以修改 `convert_deeppcb_dataset.py` 中的 `DEEPPCB_TYPE_MAP`。
...
**错误信息**: `未找到DeepPCB数据集目录` 或 `未找到任何 *_test.jpg 图像文件`
...
4. **检查路径中的中文或特殊字符**:
   ...
   ```powershell
   # Windows - 使用双引号
   python convert_deeppcb_dataset.py --deeppcb_dir "D:\数据集\DeepPCB-master"
   ```

### Q2: 标注文件格式错误

**错误信息**: `解析标注文件 xxx 第 X 行失败` 或 `无法读取标注文件`

**解决方案**:

1. **检查标注文件格式**:
   确保格式为: `x1,y1,x2,y2,type`（逗号分隔，无空格）
   ```bash
   # 查看一个标注文件示例
...
如果需要自定义转换逻辑，可以修改 `convert_deeppcb_dataset.py`:

### 修改缺陷类型映射

```python
# 在 convert_deeppcb_dataset.py 中
DEEPPCB_TYPE_MAP = {
    1: "open",
    2: "short",
    3: "open",      # 改为其他类型
...
在 `convert_deeppcb_dataset()` 函数中修改图像处理逻辑：
...
2. **检查转换结果**: 转换后务必使用 `python quick_start.py` 验证数据集格式

3. **数据集大小**: DeepPCB包含1500对图像，转换后应该有1500张测试图像

4. **训练集分割**: 默认80%训练，20%测试（1200张训练，300张测试）

5. **随机打乱**: 默认会随机打乱数据集，使用固定的随机种子（42）确保可复现
