...
try:
    ...
    from src.inference.text2vec import *
...
# 延迟初始化，避免导入时Config未准备好
_cfg_instance = None
_trr_instance = None

def _get_text_recall_rank_instance():
    """获取TextRecallRank实例（单例模式）"""
    global _cfg_instance, _trr_instance
    if _trr_instance is None:
        _cfg_instance = Config()
        _trr_instance = TextRecallRank(_cfg_instance)
    return _trr_instance

def q_searching(query):
    """查询检索函数"""
    trr = _get_text_recall_rank_instance()
    return trr.query_retrieve(query)
